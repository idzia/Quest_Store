...
public class AdminEditMentor implements HttpHandler {
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        if (Session.guard(httpExchange, "admin")) {
            ...
            if (method.equals("GET")) {
                ...
                Mentor editMentor = userDAO.getMentorById(Integer.valueOf(mentorId));

                model.with("mentor", editMentor);
                model.with("mentorId", mentorId);
            ...
            if (method.equals("POST")) {
                ...
                String mentorFirstName = inputs.get("name");
                String mentorLastName = inputs.get("surname");
                String mentorPhone = inputs.get("phone");
                String mentorEmail = inputs.get("email");


                String mentorId = parsePath(httpExchange)[2];

                UserDAO userDAO = new UserDAO();
                userDAO.updateMentor(Integer.valueOf(mentorId), mentorFirstName, mentorLastName, mentorPhone, mentorEmail);
